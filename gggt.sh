...
audit_deployment() {
    ...
    echo "$PROCS" | while read -r PROC_TYPE PROC_STATUS PROC_NAME PROC_SUBTYPE; do
        ...
        # 3. Runtime Status Check (via send command - captures map/apply parallelism details)
        ...
        # Extract Parallel Mappers / Appliers from both DIAG_OUTPUT and SEND_OUTPUT (specifically for Parallel/Integrated Replicats)
        PARALLEL_METRICS=""
        if [ "$PROC_TYPE" = "REPLICAT" ]; then
            MAP_PAR_VAL=$(echo "$SEND_OUTPUT" | grep -i "Map Parallelism" | awk -F: '{print $2}' | xargs)
            MIN_APP_VAL=$(echo "$SEND_OUTPUT" | grep -i "Min Apply Parallelism" | awk -F: '{print $2}' | xargs)
            CUR_APP_VAL=$(echo "$SEND_OUTPUT" | grep -i "Current Apply Parallelism" | awk -F: '{print $2}' | xargs)
            MAX_APP_VAL=$(echo "$SEND_OUTPUT" | grep -i "Max Apply Parallelism" | awk -F: '{print $2}' | xargs)
            ACT_APP_VAL=$(echo "$SEND_OUTPUT" | grep -i "Active Appliers" | awk -F: '{print $2}' | xargs)

            # Fallback configuration lookups if send output missed them
            if [ -z "$CUR_APP_VAL" ]; then
                CUR_APP_VAL=$(echo "$DIAG_OUTPUT" | grep -iE "(Appliers|Applier Threads|Coordinator)" | head -n 1 | awk -F: '{print $2}' | xargs)
            fi

            PARALLEL_METRICS="Mappers: ${MAP_PAR_VAL:-N/A} | Appliers (Min/Cur/Max): ${MIN_APP_VAL:-N/A}/${CUR_APP_VAL:-N/A}/${MAX_APP_VAL:-N/A} | Active: ${ACT_APP_VAL:-N/A}"
        fi

        # Generate Visual Bar Symbol Graph based on lag and workload activity
        LAG_SYMBOL_BAR=""
        BAR_LEN=$(( TOTAL_LAG_SECS / 30 ))
        [ $BAR_LEN -gt 10 ] && BAR_LEN=10
        [ $BAR_LEN -lt 1 ] && [ $TOTAL_LAG_SECS -gt 0 ] && BAR_LEN=1
        
        if [ "$TOTAL_LAG_SECS" -eq 0 ]; then
            LAG_SYMBOL_BAR="[🟢 OK 0s]"
        else
            FILLED_BAR=$(printf '█%.0s' $(seq 1 $BAR_LEN))
            EMPTY_BAR=$(printf '░%.0s' $(seq 1 $((10 - BAR_LEN))))
            if [ "$TOTAL_LAG_SECS" -gt "$MAX_CHECKPOINT_LAG_SECS" ]; then
                LAG_SYMBOL_BAR="[🔴 LAG: ${FILLED_BAR}${EMPTY_BAR} ${TOTAL_LAG_SECS}s]"
            else
                LAG_SYMBOL_BAR="[🟡 WN: ${FILLED_BAR}${EMPTY_BAR} ${TOTAL_LAG_SECS}s]"
            fi
        fi

        # Throughput activity graph ticker
        OPS_SYMBOL_BAR=""
        OPS_LEN=$(( TOTAL_OPS / 10 ))
        [ $OPS_LEN -gt 10 ] && OPS_LEN=10
        [ $OPS_LEN -lt 1 ] && [ $TOTAL_OPS -gt 0 ] && OPS_LEN=1
        THRP_BAR=$(printf '■%.0s' $(seq 1 $OPS_LEN))
        OPS_SYMBOL_BAR="[⚡ Ops: $TOTAL_OPS $THRP_BAR]"

        # Display Metrics Block
        printf "  %-25s : %s\n" "Checkpoint Lag" "${LAG_STR:-00:00:00} (${TOTAL_LAG_SECS}s)"
        printf "  %-25s : %s\n" "Lag Trend Symbol" "$LAG_SYMBOL_BAR"
        ...
        printf "  %-25s : %s\n" "Throughput Symbol" "$OPS_SYMBOL_BAR"
        if [ -n "$PARALLEL_METRICS" ]; then
            printf "  %-25s : %s\n" "Parallel Threads" "$PARALLEL_METRICS"
        fi
        printf "  %-25s : %s\n" "Data Processing State" "$ACTIVITY_STATUS"
        printf "  %-25s : %s\n" "Backlog Evaluation" "$BACKLOG_STATUS"
        printf "  %-25s : %s\n" "Process Health" "$HEALTH"

        # Clean runtime status output handling
        CLEAN_SEND=$(echo "$SEND_OUTPUT" | grep -v "adminclient" | grep -v "Successfully logged" | grep -v "OGG (" | grep -v -- "---" | grep -v "^$" | head -n 4 | xargs)
